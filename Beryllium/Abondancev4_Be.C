...
int main()
{
	...
	double pi=3.14159;
	double writing_variable;
	double Z_x_old;
	//~ ofstream output_De("results_Graphe_De.dat");
	//~ ofstream output_He("results_Graphe_He.dat");
	ofstream output_Be("OutputBe/results_Graphe_Be_spectre_standard.dat");
	double M_x,E_max;
	M_x=4.;
	E_0=M_x/2;
	//~ T=1*pow(10,-1);
	...
	for(int h=1;h<h_max;h++)
	{
		...
		for(int i=7;i<8;i++)
		{
			...
			//~ if(Ec>3)Ec=3;
				...
				s=qsimp(func_Be_non_standard,z_max,z,z,i,E_0);
