...
int main()
{
    ...
    ofstream output_Check("../OutputExtra/Interpolation_3He.dat");
    ...
    tau=2.365*pow(10,4);double tau_moy=2.365*pow(10,6);double tau_moy2=2.365*pow(10,8);double tau_max=2.365*pow(10,10);
    //~ double T_min = 0.511*0.511/(1.6*22);
    double T_max = pow(tau_max*(2*H_r),-0.5)*T_0;
    //~ double T_max = 0.511*0.511/(2.2*22);
    //~ tau=2.365*pow(T_min/T_0,-2)/(2*H_r);tau_max=pow(T_max/T_0,-2)/(2*H_r);
  ...
  double s_3He_produc=0, s_3He_destruc=0, s_4He_destruc =0;
    double  y, dy;
    ...
    for(int h=0;h<h_max;h++)
    {
        ...
        s_3He_produc=y;
        output_Check  << log10(z) << " " << s_4He_destruc << " " << s_3He_destruc << " " << s_3He_produc << endl;
          }
      output_Check.close();
