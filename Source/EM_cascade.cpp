...
double  dsigma_compton(double  x, double  z, double g){
	...
	double  dsigma =0;
	dsigma = pi*pow(r_e,2)*m_e/pow(x,2)*(x/g+g/x+pow(m_e/g-m_e/x,2)-2*m_e*(1/g-1/x))*n_e*pow(1+z,3)/(1+Y);
...
void Spectrum_electron_scattered(struct Structure_Particle_Physics_Model * pt_Particle_Physics_Model,
	  	...
	  	double z = pt_Input_Electron_Spectrum->redshift;
		 	int n_step = pt_Spectrum_and_Precision_Parameters->n_step*100;
		 	double E_0 = pt_Particle_Physics_Model->E_0;
		 	double E_gamma_bb = 2.701*T_0*(1+z);
		 	double E_x = E_x_0/(1+z), E_c = E_c_0/(1+z);
		 	double int_bb = 2*pow(T_0*(1+z),3)*1.20205/(pi*pi);
          ...
          resultat+= dE_2/840. * (41*F1+216*F2+27*F3+272*F4+27*F5+216*F6+41*F7);
				...
				else pt_Output_Electron_Spectrum->Spectrum[j]=resultat*2*pi*r_e*r_e*m_e*m_e*int_bb/(E_gamma_bb);
					...
					if(E_c <= pt_Particle_Physics_Model->E_0){
								...
								if(pt_Spectrum_and_Precision_Parameters->spectrum_mode == "writing"){
									...
									if(pt_Spectrum_and_Precision_Parameters->electron_spectrum_choice == "none"){
											for(int i=0;i<Gamma_Table_Size;i++){
												E1=E_min+i*dE;
												Inverse_Compton_Spectrum.Energy[i]=E1;
												Inverse_Compton_Spectrum.Spectrum[i]=0;
												Electron_Spectrum.Energy[i]=E1;
												Electron_Spectrum.Spectrum[i]=0;
											}
									}
									...
									Electron_Spectrum.Spectrum[j]=pt_Spectrum_and_Precision_Parameters->Injected_Electron_Spectrum(E1,z,pt_Particle_Physics_Model->E_0)+Diffused_Electron_Spectrum.Spectrum[j];
									}
									}
									if(pt_Spectrum_and_Precision_Parameters->spectrum_mode == "writing"){
								...
								for(int j=0; j<Electron_Table_Size; j++){
									Electron_Spectrum.Spectrum[j]+=	Compton_Electron_Spectrum.Spectrum[j];
								}
								 Spectrum_electron_scattered(pt_Particle_Physics_Model,
																					 	pt_Spectrum_and_Precision_Parameters,
																						&Electron_Spectrum,
																						&Diffused_Electron_Spectrum);
								for(int j=0; j<Electron_Table_Size; j++){
									cout << " Compton_Electron_Spectrum = " << Compton_Electron_Spectrum.Spectrum[j] << " Diffused_Electron_Spectrum = " << Diffused_Electron_Spectrum.Spectrum[j]<<endl;
									Compton_Electron_Spectrum.Spectrum[j]+= Diffused_Electron_Spectrum.Spectrum[j];
									Electron_Spectrum.Spectrum[j]+=Diffused_Electron_Spectrum.Spectrum[j];
								}
								//  check_energy_conservation(pt_Particle_Physics_Model,pt_Spectrum_and_Precision_Parameters,pt_Cascade_Spectrum,&Electron_Spectrum,integrale);
									...
									Spectre_gamma_compton(pt_Particle_Physics_Model,
																				...
																				&Inverse_Compton_Spectrum);
									for(int i = 0; i<Gamma_Table_Size ; i++){
										cout <<"pt_Cascade_Spectrum->Spectrum[i] = " << pt_Cascade_Spectrum->Spectrum[i] << " Inverse_Compton_Spectrum.Spectrum[i] = " << Inverse_Compton_Spectrum.Spectrum[i] << endl;
										pt_Cascade_Spectrum->Spectrum[i]+=Inverse_Compton_Spectrum.Spectrum[i];
									}
									if(pt_Spectrum_and_Precision_Parameters->spectrum_mode == "writing"){
										Compton_Electron_Spectrum.spectrum_name = "compton_electron_";
